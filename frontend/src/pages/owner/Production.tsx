...
function ProductionInner() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch production jobs and completed orders in parallel
      const [response, completedResponse] = await Promise.all([
        getProductionJobs(),
        getOrders({ status: 'COMPLETED' }),
      ]);
      const jobList = (response.data.data || []).map((job: any) =>
        mapJob(job, i18n.language)
      );
      setJobs(jobList);
      setCompletedOrders(completedResponse.data.data.length);
  ...
  return (
    <AppShell role="owner" activePage="owner-production">
      ...
      <section className="grid-4" style={{ marginBottom: 14 }}>
        <StatCard
          label={t('production:stats.activeJobs')}
          value={activeJobs}
          sub={t('production:stats.activeSub')}
        />
        <StatCard
          label={t('production:stats.ready')}
          value={ready}
          sub={t('production:stats.readySub')}
        />
        <StatCard
          label={t('production:stats.inProgress')}
          value={inProgress}
          sub={t('production:stats.inProgressSub')}
        />
        <StatCard
          label={t('production:stats.completed')}
          value={completed}
          sub={t('production:stats.completedSub')}
        />
        <StatCard
          label={t('production:stats.completedOrders')}
          value={completedOrders}
          sub={t('production:stats.completedOrdersSub')}
        />
      </section>
              ...
              <div className="search-container">
                ...
                {dropdownOpen && (
                  <div className="filter-dropdown show">
                    <div className="field">
                      ...
                      >
                        ...
                        <option value="ready_for_production">
                          {t('production:steps.pending')}
                        </option>
                        <option value="design">
                          {t('production:steps.design')}
                        </option>
                        <option value="printing">
                          {t('production:steps.printing')}
                        </option>
                        <option value="cutting">
                          {t('production:steps.cutting')}
                        </option>
                        <option value="packaging">
                          {t('production:steps.packaging')}
                        </option>
                        <option value="completed">
                          {t('production:steps.ready')}
                        </option>
                      </select>
                    </div>
                    <button
                    ...
                    >
                      {t('production:table.filter.apply')}
                    </button>
                  </div>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p style={{ marginBottom: 2 }}>
                        <strong>{t('production:card.order')}:</strong> #
                        {job.orderId}
                      </p>
                      <p style={{ marginBottom: 2 }}>
                        <strong>{t('production:card.client')}:</strong>{' '}
                        {job.client}
                      </p>
                      <p style={{ marginBottom: 2 }}>
                        <strong>{t('production:card.product')}:</strong>{' '}
                        {job.product} -{' '}
                        {t('production:card.pcs', { count: job.qty })}
                      </p>
                      <p style={{ marginBottom: 6 }}>
                        <strong>{t('production:card.step')}:</strong>{' '}
                        {t(`production:steps.${job.currentStep}`)}
                      </p>
                      <ProgressBar
                        percent={progress}
                      ...
                      >
                        {t('production:card.completedOf', {
                          completed: job.completedQty,
                          total: job.qty,
                          percent: progress,
                        })}
                      </p>
        ...
        {selectedJob && (
          ...
          >
            ...
            <h4 style={{ margin: '12px 0 8px' }}>
              {t('production:detail.jobDetails')}
            </h4>
            <ul
              style={{
                listStyle: 'none',
            ...
            >
              <li>
                <strong>{t('production:detail.product')}:</strong>{' '}
                {selectedJob.product}
              </li>
              <li>
                <strong>{t('production:detail.quantity')}:</strong>{' '}
                {t('production:card.pcs', { count: selectedJob.qty })}
              </li>
              <li>
                <strong>{t('production:detail.orderStatus')}:</strong>{' '}
                {selectedJob.orderStatus}
              </li>
              <li>
                <strong>{t('production:detail.dueDate')}:</strong>{' '}
                {selectedJob.dueDate}
              </li>
            </ul>
            ...
            <h4 style={{ margin: '12px 0 8px' }}>
              {t('production:detail.progress')}
            </h4>
            <ProgressBar
              percent={getProgress(selectedJob)}
              color={progressColor(getProgress(selectedJob))}
            />
            <p
              style={{
                fontSize: 12,
            ...
            >
              {t('production:detail.completedOf', {
                completed: selectedJob.completedQty,
                total: selectedJob.qty,
                percent: getProgress(selectedJob),
              })}
            </p>
            ...
            <h4 style={{ margin: '12px 0 8px' }}>
              {t('production:detail.updateStep')}
            </h4>
        ...
        >
          ...
          >
            ...
            >
              ...
              <button className="btn" onClick={() => setShowWorkView(false)}>
                {t('production:workView.close')}
              </button>
            </div>
            <div style={{ padding: 20 }}>
              <h3 style={{ marginBottom: 8 }}>
                {t('production:workView.progressTitle', {
                  id: selectedJob.id,
                })}
              </h3>
              <ProgressBar
              ...
              >
                {t('production:workView.completedOf', {
                  completed: selectedJob.completedQty,
                  total: selectedJob.qty,
                  percent: getProgress(selectedJob),
                })}
              ...
              <h3 style={{ marginBottom: 12 }}>
                {t('production:workView.jobInfo')}
              </h3>
              <div className="form-grid-2" style={{ fontSize: 14, gap: 8 }}>
                <p>
                  <strong>{t('production:workView.client')}:</strong>{' '}
                  {selectedJob.client}
                </p>
                <p>
                  <strong>{t('production:workView.order')}:</strong> #
                  {selectedJob.orderId}
                </p>
                <p>
                  <strong>{t('production:workView.product')}:</strong>{' '}
                  {selectedJob.product}
                </p>
                <p>
                  <strong>{t('production:workView.quantity')}:</strong>{' '}
                  {selectedJob.qty}
                </p>
                <p>
                  <strong>{t('production:workView.status')}:</strong>{' '}
                  <StatusBadge status={selectedJob.status} />
                </p>
                <p>
                  <strong>{t('production:workView.currentStep')}:</strong>{' '}
                  {t(`production:steps.${selectedJob.currentStep}`)}
                </p>
                <p>
                  <strong>{t('production:workView.deadline')}:</strong>{' '}
                  {selectedJob.dueDate}
                </p>
                <p style={{ gridColumn: '1 / -1' }}>
                  <strong>{t('production:workView.notes')}:</strong>{' '}
                  {selectedJob.notes}
                </p>
              </div>
            </div>
